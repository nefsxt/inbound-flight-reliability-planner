...
from huggingface_hub.utils import RepositoryNotFoundError, HfHubHTTPError  # NEW: HfHubHTTPError needed to give a clear error message on auth/access failures
...
def download_from_hf(local_dir: str = ".") -> str:
    ...
    # NEW: snapshot_download's own errors on a missing/invalid token or an
    # inaccessible repo are often a generic HTTP 401/403 with little
    # context -- this is the failure mode the Streamlit app hits when
    # DATA_SOURCE=hf is set but HF_TOKEN isn't configured (a *separate*
    # secrets store from GitHub Actions secrets, so it's easy to set one
    # and forget the other). Wrapping it here gives every caller
    # (including the app) a clear, actionable message instead of a raw
    ...
    except HfHubHTTPError as exc:  # NEW
        if not token:  # NEW
            raise RuntimeError(  # NEW
                f"Could not download from Hugging Face repo '{config.HF_REPO_ID}': "
                "no HF_TOKEN environment variable is set. If this is a Streamlit "
                "Cloud deployment, note that GitHub Actions secrets and Streamlit "
                "Cloud secrets are separate -- HF_TOKEN must be added under the "
                "app's own Settings -> Secrets."
            ) from exc
        raise RuntimeError(  # NEW
            f"Could not download from Hugging Face repo '{config.HF_REPO_ID}' "
            f"(repo_type={config.HF_REPO_TYPE}) even though HF_TOKEN is set. "
            "Check that the token has read access to this exact repo and that "
            f"the repo/type/ID are correct: {exc}"
        ) from exc
